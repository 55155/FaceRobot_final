...
path = 'C:/Users/USER/Desktop/aispeaker test/'
if language == "Kor" :
    prompt_text = "다음은 고정된 플랫폼에 머리만 있는 애니메트로닉스 로봇 RAY와의 대화입니다. RAY는 매우 친절하고 똑똑하지만 날씨와 시간 같은 실시간 정보는 모릅니다. RAY는 한국에 살고 있지만, 스웨덴을 정말 가보고 싶다고 합니다.: \nYou:나랑 이야기하자!\nRAY:좋아요. 저와 재미있게 놀아요."
elif language == "En" :
    prompt_text="The following is a conversation with Ray, animatronics robot with only a head on a fixed platform. Ray is very kind and smart but doesn't know the real-time information like weather and time. Ray lives in Korea, but Ray thinks Sweden is a country that Ray really wants to visit.: \nYou:Talk to me!\nRAY:All right. Have fun with me."
            ...
            if result.is_final:
                ...
                if "WAITING_FOR_USER_INPUT" in status :
                    if language == "Kor" : 
                        ...
                        song_condition = "노래" in transcript and "해줘" in transcript
                        song_ment = "알겠어요. 제가 준비한 노래를 들려드릴게요!"
                        complete_song_ment = "(Ray가 노래를 부른다.) 어때요? 잘 감상하셨나요? 노래가 듣고 싶다면 언제든지 얘기해도 좋아요."
                        introduce_condition = "자기소개" in transcript and "해줘" in transcript
                        ...
                        complete_song_ment = "(Ray sings a song.) How was it? Did you enjoy it? You can tell me whenever you want to listen to the song."
                    ...
                    if exit_condition:
                        ...
                        global prompt_text
                        prompt_text = prompt_text +  "\nYou:" + transcript + "\nRAY:" + song_ment + complete_song_ment
                        with open(path + "data_sync.txt", "w") as f:
                            f.write("SING_A_SONG")
