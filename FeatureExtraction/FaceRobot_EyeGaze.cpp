		...
		auto open_face = [&]()
			{
				...
				while (!captured_image.empty()) {
					...
					if (abs(ec.y) < abs(ec.x)) {
						...
						if (gazeAngle(1) < 0) {
							_gamma = (-P_c.y + upperplane) / ec.y;
						}
						else {
							_gamma = (-P_c.y + lowerplane) / ec.y;
						}
					}
					else {
					...
					cv::Point3f gaze_point_cam = P_c + _gamma * ec;
					...
					if (0 < gaze_point_cam.z && gaze_point_cam.z < P_c.z) { // gaze 해야하는 경우  rotation matrix
						...
						cout << gaze_point_robot << endl;
					}


					pH(0) = Poc_x + gaze_point_robot.x;
					pH(1) = (Poc_y + gaze_point_robot.y) * 0.75;		// 카메라 or OpenFace가 조금 부정확한 거 같아서 임의로 튜닝

					if (Poc_z + gaze_point_robot.z > 0)
						pH(2) = (Poc_z + gaze_point_robot.z) * 0.9;	// 카메라 or OpenFace가 조금 부정확한 거 같아서 임의로 튜닝
					else
						pH(2) = (Poc_z + gaze_point_robot.z) * 0.7;	// 카메라 or OpenFace가 조금 부정확한 거 같아서 임의로 튜닝
		...
		auto dynamixel_control = [&]()
			{
				...
				while (!captured_image.empty()) {
					...
					if (flag_write || run_iteration == 1) {
						...
						if (!moveDXLtoDesiredPosition(portHandler, packetHandler, dxl_goal_position)) return 0;
