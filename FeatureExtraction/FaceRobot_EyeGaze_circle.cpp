...
double deque_mean(std::deque<double> q) {
	double sum = std::accumulate(q.begin(), q.end(), 0);
...
int main(int argc, char** argv)
{
	...
	deque<double> Y; // 로봇 시점 Y data 50개 slide window
	deque<double> Z; // 로봇 시점 Z data 50개 slide window
	double avg_Y;
	double avg_Z;
	double val_Y = 1e9;
	double val_Z;
	double N = 25; // data set의 개수
		...
		auto open_face = [&]()
			{
				...
				while (!captured_image.empty()) {
					...
					// define the plane
					double R = 400; // 적절한 위치에 생성해야함.
					// 눈 위치를 중심으로 원을 그리는 가정
					VectorXd O = robot_to_cam(0.0, 0.0, EYE_LAYER_HEIGHT + 100);
					VectorXd _gamma(2);
					...
					// solution
					if (a != 0) {
						_gamma(0) = (-b + sqrt(pow(b, 2) - 4 * a * c)) / (2 * a);
						_gamma(1) = (-b - sqrt(pow(b, 2) - 4 * a * c)) / (2 * a);

						// solution1
						x[0] = P_c.x + _gamma(0) * ec.x;
						y[0] = P_c.y + _gamma(0) * ec.y;
						z[0] = P_c.z + _gamma(0) * ec.z;

						// solution2
						x[1] = P_c.x + _gamma(1) * ec.x;
						y[1] = P_c.y + _gamma(1) * ec.y;
						z[1] = P_c.z + _gamma(1) * ec.z;

						// z 값이 더 작은 gamma 값을 추종해야함
						gaze_point_cam = (z[0] < z[1]) ? P_c + _gamma(0) * ec : P_c + _gamma(1) * ec;
					}
					else { // zerodivision
					...
					// 10cm 정도 떨어진 지점. 
					if (100 < gaze_point_cam.z && gaze_point_cam.z < P_c.z - 100) { // gaze 해야하는 경우  rotation matrix
						...
						if (Y.size() < N && Z.size() < N && flag_first_cal) {
							Y.push_back(gaze_point_robot.y);
							Z.push_back(gaze_point_robot.z);
						...
						// 처음 50개의 데이터가 들어왔을 때
						else if (Y.size() == N && Z.size() == N && flag_first_cal) {
							velocity = 500;
							...
							val_Y = deque_variance(Y);
							val_Z = deque_variance(Z);
						}
						else {
							...
							double X_kn = Y.front();
							Y.pop_front();
							double X_k = gaze_point_robot.y;
							Y.push_back(X_k);

							double pre_avg = avg_Y;
							double pre_val = val_Y;

							avg_Y = Moving_Avg_Filter(X_k, pre_avg, X_kn);
							val_Y = recursive_Variance(X_k, pre_avg, pre_val, X_kn);
							
							cout << "val_Y : " << val_Y << endl;
						}
					...
					double threshold_var = 100;
					// 2초동안 어딘가를 응시하고 있으면서, 그 분산이 크지 않다면, 
					if (val_Y < threshold_var && Y.size() == N && !flag_first_cal) {
						cout << "val_Y : " << val_Y << endl;
						/*
						while (!Y.empty()) {
							Y.pop_back();
					...
					visualizer.SetObservationGaze(gaze_point_cam, gaze_point_cam, LandmarkDetector::CalculateAllEyeLandmarks(face_model), LandmarkDetector::Calculate3DEyeLandmarks(face_model, sequence_reader.fx, sequence_reader.fy, sequence_reader.cx, sequence_reader.cy), face_model.detection_certainty);
