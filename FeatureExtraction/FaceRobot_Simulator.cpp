		...
		auto open_face = [&]()
			{
				...
				while (!captured_image.empty()) {
					...
					// sampling
					double T = 4000;
					double theta = (threaditer * 40 / T) * (2 * M_PI);
					
					gaze_point_robot.x = 100 + threaditer / 2;
					gaze_point_robot.y = R * cos(theta);
					gaze_point_robot.z = R * sin(theta) + EYE_LAYER_HEIGHT;
		...
		auto dynamixel_control = [&]()
			{
				...
				while (!captured_image.empty()) {
					...
					if (flag_val_all_smaller_than_thr && flag_grad_all_smaller_than_thr) {
						flag_write = true;
