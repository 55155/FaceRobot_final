...
int main(int argc, char** argv)
{
	...
	string audioname = "dictator";

	string audiofilepath = folderpath + "audio\\" + audioname + ".wav"; 
	string Headcsvfilepath = folderpath + "csv\\good\\" + audioname + ".csv";
	string Mouthcsvfilepath = folderpath + "csv\\good\\" + audioname + "-delta-big.csv";
	string segmentfolderpath = folderpath + "segments\\";
	string originalMusicPath = audiofilepath;

	//string originalMusicPath = folderpath + "audio\\" + audioname + "_original.wav";
	...
	while (true)
	{
		...
		// open_face thread에서 나오는 변수들 중에 dynamixel_control thread에서 사용할 변수들: roll_OF, pitch_OF, yaw_OF, mouth_OF, confidence
		auto open_face = [&]()
		{
